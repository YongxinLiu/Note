...
#默认按照门水平上色，标签为门水平标签
  ...
  # colnames(tax_point) = paste("label",colnames(tax_point),sep = "_")
  # data_plot = merge(data_plot,tax_point,by = "row.names",all = TRUE)
  # row.names(data_plot) = data_plot$Row.names
  # data_plot$Row.names = NULL
  # head(data_plot)
  ...
  ggsave("./maptree1.pdf", p, width = 12, height =10, device = cairo_pdf, family = "Times New Roman" )
  ...
  ggsave("./maptree2.pdf", p, width = 12, height =10, device = cairo_pdf, family = "Times New Roman" )
