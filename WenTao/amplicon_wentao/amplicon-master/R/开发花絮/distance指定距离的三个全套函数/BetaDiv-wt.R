...
print("3")
title1 = MicroTest(ps  = ps1_rela,dist = dist,Micromet = Micromet)
print("3")
title1
# 三种方法两两不比较
source("../pairMicroTest-wt.R")
pairResult = pairMicroTest(ps = ps1_rela,dist = dist,Micromet = Micromet)
...
if (method %in% c("DCA", "CCA", "RDA",  "MDS", "PCoA","PCA","LDA")) {
  p2 <-ggplot(points, aes(x=x, y=y, fill = Group)) +
    ...
    labs(x=paste(method," 1 (", format(100 * eig[1] / sum(eig), digits=4), "%)", sep=""),
         y=paste(method," 2 (", format(100 * eig[2] / sum(eig), digits=4), "%)", sep=""),
         title=title1)+
