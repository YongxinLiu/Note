...
$scale=1000000/$opts{r};
print "bedtools genomecov -ibam $opts{i} -scale $scale -bg>$opts{o}\.bedgraph\n";
`bedtools genomecov -ibam $opts{i} -scale $scale -bg>$opts{o}\.bedgraph`;
...
sub usage {
    die(
        qq/
Usage:    format_bam2tdf.pl -i inpute_file_bam -o output_file_tdf -r mapped_total_reads -g genome_name
