...
#1	1745	1768	TTTAAGTGGCTTCTTCATGGTTCT	2	-
my %top;#keep each src seq and reads
open OUTPUTR,">$opts{o}";
...
$loci=1;
$reads=$tmp[4];
$reads_norm=($tmp[4]/$database{$tmp[3]});
$top{$tmp[3]}=$tmp[4]; 
$reads_watson=$reads_norm if $tmp[5] eq "+";
$len=length($tmp[3]);
...
#	$tmp[0]=~/(\d+)&(\d+)/;#get id in $1 and reads number in $2
	next if $tmp[4]<$opts{r};#filter low than $opts{r} reads sequences
	next if $database{$tmp[3]}>$opts{l};#filter more than $opts{l} hits sequences
#	if ($opts{x} eq "y") {
...
#		print OUTPUT "$1\t$2\t$tmp[0]\t$tmp[2]\t$tmp[3]\t$tmp[4]\t$database{$tmp[5]}\t$tmp[5]\n";#input select mapping loci
#		close OUTPUT;
#	}
	if ($chr eq $tmp[0] && $tmp[3]-$end>$opts{s}+1) {#In the same chr and distance more than threshold of distance($opts{s})
		$length=$end-$start+1;
		...
		if ($loci>=$opts{m} && $reads>=$opts{n} && $rpm>=$opts{p} && $rpkm>=$opts{k}) {
			@seq=sort {$top{$b}<=>$top{$a};}  keys %top; #all sequence sort by reads descend
			printf OUTPUTR "$id\t$chr\t$strand\t$start\t$end\t$length\t$loci\t$reads\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t$seq[0]\t$top{$seq[0]}\t$seq[1]\t$top{$seq[1]}\t$seq[2]\t$top{$seq[2]}\n",$reads_norm,$rpm,$rpkm,$length{20},$length{21},$length{22},$length{23},$length{24},$propotion;
		...
		$len=length($tmp[3]);
		$length{$len}+=$reads_norm;
	}elsif ($chr eq $tmp[0] && $tmp[3]-$end<=$opts{s}+1){#In the same cluster
		$start=$tmp[1] if $tmp[1]<$start;
		$end=$tmp[2] if $tmp[2]>$end;
		$loci++;
		$reads+=$tmp[4];
		$reads_norm=($tmp[4]/$database{$tmp[3]});
		$top{$tmp[3]}=$tmp[4];
...
sub usage {
    die(
        qq/
Usage:    define_cluster6.1.pl -i inpute_file -d loci_file -o output_file
...
Notes:    v2.0 add threshold of loci, reads, RPKM
		  ...
		  v6.1 add each SRCs top 3 high expressed sRNA
\n/
    )
}