...
while (<INPUT>) {
	...
	my @tmp=split/\t/;
	if ($tmp[2]=~/primary/) {
		$tmp[8]=~/ID=([\w_]+);accession_number=([\w]+);Name=([\w-]+)/;
#		print "$1\t$3\n";
		$pre{$1}{id}=$3;
		$pre{$1}{start}=$tmp[3];
		$pre{$1}{end}=$tmp[4];
	}elsif ($tmp[8]=~/derives/){
		$tmp[8]=~/Name=([\w\.-]+);derives_from=([\w_]+)/;
		$preid=$2;
#		print "$1\t$2\n";
		if ($tmp[6] eq '+') {
...
sub usage {
    die(
        qq/
Usage:    format_mirChr_mirPre19.pl.pl -i miR gff3 -o mir Bed
Function: Format mature miRNA chromosome position to pre-miRNA position, based on miRbase gff3 file of miRbase19
