...
open OUTPUT,">$opts{o}";
#chr0	start1	end2	width3	strand4
#chr1	1	284	284	+
#chr1	5	205	201	+
#chr1	5	205	201	+
my %pair;
...
while (<INPUT>) {
	...
	if (defined($pair{$tmp[3]}{chr})) {
		...
		print OUTPUT "$tmp[0]\t",$pair{$tmp[3]}{start}-$opts{s},"\t",$tmp[2]-$opts{s},"\t",$tmp[2]-$pair{$tmp[3]}{start}+1,"\t\+\n";
	}else{
...
sub usage {
    die(
        qq/
Usage:    format_bed2nucleR.pl -i bed(line4_is_reads) -o nucleR
