...
Notes:    1.0 outpu html report and logo
          ...
          1.6 Add english version for first, shorten menu chinese
          1.7 Add mterial list, fully check grammar
          1.8 Add group3 CPCoA 2017/11/7
          1.9 If detect group_venn and group_tern, and report result 2017/12/25
          2.0 Add rarefraction, PCA and LDA 2017/12/28
\n!
    )
}
...
table_design=head(table_design,n=3)
knitr::kable(table_design, caption="样品信息前3行。Top 3 lines of sample information.", booktabs=TRUE)
```
!;
close OUTPUT;
...
tab=head(tab,n=3)
knitr::kable(tab, caption="OTU表前3行。Top 3 lines of OTU table.", booktabs=TRUE)
```

原始数据经过去冗余、只保留出现8次以上的序列、使用unoise3进行去噪(非97%聚类，相当于100%聚类)、自身比对去嵌合体、与RDP数据比对去嵌合体、与GreenGene 13.5(可选RDP11.5/SILVA128)比对去除非细菌序列。原始含分类学注释OTU表下载[BIOM](result/otu_table_tax.biom) [TXT](result/otu_table_tax.txt)，[代表性序列Fasta](result/rep_seqs.fa)。[代表性序列与培养菌资源库对应列表](result/otu_cultured.txt)。基本统计信息如下：

**重要原始数据下载**

1. 原始OTU表(含Taxonomy)下载[BIOM](result/otu_table_tax.biom)、[TXT](result/otu_table_tax.txt)；
2. OTU代表性序列[FASTA](result/rep_seqs.fa)；
3. OTU物种注释列表[TXT](result/rep_seqs_tax_assignments.txt)。
...
原始OTU表去除叶绿体纲(c__Chloroplast)和线粒体科(f__mitochondria)，它们可能为叶绿体/线粒体来源(ITS去除宿主)后，只保留在至少在一个样品中存在丰度大于$opts{a}的OTU，接下来的数据进行差异统计分析。Raw OTU remove Cyanobacteria and  abundance lower than $opts{a}. [BIOM](result_k1-c/otu_table_tax.biom) [TXT](result_k1-c/otu_table_tax.txt) [Normalized OTU table in percentage](result_k1-c/otu_table_norm.txt)，[代表性序列Fasta](result_k1-c/rep_seqs.fa)。Basic statistics information：
...
(ref:sample-cor) 基于各样品OTUs相对丰度计算皮尔森相关系数。Pearson correlation of all samples. [PDF](result_k1-c/heat_cor_samples.pdf) [TXT](result_k1-c/sim_samples.txt)
...
(ref:group-cor) 基于各样品组OTUs均值计算皮尔森相关系数。Pearson correlation of all groups (mean). [PDF](result_k1-c/heat_cor_groups.pdf) [TXT](result_k1-c/sim_groups.txt)
...
(ref:group-corplot) 基于各样品组OTUs均值计算皮尔森相关系数绘制corplot。Pearson correlation of all groups (mean) in corplot. [PDF](result_k1-c/corplot_pie_pearson.pdf) [TXT](result_k1-c/sim_groups.txt)

```{r group-corplot, fig.cap="(ref:group-corplot)", out.width="99%"}
knitr::include_graphics("result_k1-c/corplot_pie_pearson.png")
```

### 样品组bray距离矩阵corplot

(ref:group-bray) 基于各样品组OTUs均值计算皮尔森相关系数。Pearson correlation of all groups (mean). [PDF](result_k1-c/corplot_pie_vegan_bray.pdf) [TXT](result_k1-c/bray_groups.txt)

```{r group-bray, fig.cap="(ref:group-bray)", out.width="99%"}
knitr::include_graphics("result_k1-c/corplot_pie_vegan_bray.png")
```
...
### Alpha多样性箱线图 {#result-diversity-alpha-figure}
...
### Alpha多样性稀释曲线 {#result-diversity-alpha-rare}
...
Rarefraction curve of richness (Observed OTUs) in samples and groups. [TXT](result/alpha_rare.txt)  [Samples PDF](result/alpha_rare_samples.pdf)  [Groups PDF](result/alpha_rare_groups.pdf)  

```{r div-rare, fig.cap="(ref:div-rare)", out.width="99%"}
figs_2 = paste0("result/alpha_rare_", c("samples", "groups"),".png")
knitr::include_graphics(figs_2)
```
...
### 组内距离箱线图 {#result-diversity-dis-boxplot}
#
#(ref:dis-boxplot) 箱线图展示组内样品间距离分布。Boxplot showing inner distance of group. [PDF](result_k1-c/beta_boxplot_bray_curtis.pdf)
#
#```{r dis-boxplot, fig.cap="(ref:dis-boxplot)", out.width="99%"}
#figs_2 = paste0("result_k1-c/beta_pcoa_", c("bray_curtis", "unweighted_unifrac", "weighted_unifrac"),".png")
#if ($opts{S}) {figs_2 = paste0("result_k1-c/beta_boxplot_", c("bray_curtis"),".png")
#}else{figs_2 = paste0("result_k1-c/beta_pcoa_", c("bray_curtis", "unweighted_unifrac", "weighted_unifrac"),".png")}
#knitr::include_graphics(figs_2)
#```

$file = "result_k1-c/beta_pca.pdf";
if (-e $file) {
print OUTPUT qq!
## PCA样品间差异Principal Component Analysis

(ref:div-pca) 主成分分析(PCA)展示第1/2坐标轴下各样品间微生物组差异(dissimilarity)。
Principal Component Analysis (PCA) samples differences. [PDF](result_k1-c/beta_pca.pdf)

```{r div-pca, fig.cap="(ref:div-pca)", out.width="99%"}
figs_2 = paste0("result_k1-c/beta_", c("pca"),".png")
knitr::include_graphics(figs_2)
```
!;
}
...
$file = "result_k1-c/CLDA_$opts{g}.pdf";
if (-e $file) {
print OUTPUT qq!
## 线性判别分析组间差异LDA

(ref:div-lda) 线性差别分析(LDA)展示第1/2坐标轴下各组间微生物组差异(dissimilarity)。
Linear Discriminant Analysis (LDA) groups differences. [PDF](result_k1-c/CLDA_$opts{g}.pdf)

```{r div-lda, fig.cap="(ref:div-lda)", out.width="99%"}
figs_2 = paste0("result_k1-c/CLDA_$opts{g}",".png")
knitr::include_graphics(figs_2)
```
!;
}

# 全部和整体差异不大，暂不显示，抓主要矛盾即可
...
e = read.table("result_k1-c/phylumpro_$tmp[0]vs$tmp[1]_enriched.xls", sep="\t", header=T)
e = head(e[order(-e\$A_mean),],n=10)
d = read.table("result_k1-c/phylumpro_$tmp[0]vs$tmp[1]_depleted.xls", sep="\t", header=T)
d = head(d[order(-d\$B_mean),],n=10)
m = rbind(e,d)
knitr::kable(m,row.names=F, caption="样品组$tmp[0] vs $tmp[1]显著差异门及变形菌纲前10行；Significantlly different phylumpro.", booktabs=TRUE)
```
...
[Volcano plot PDF](result_k1-c/vol_otu_$tmp[0]vs$tmp[1].pdf)  [Heatmap PDF](result_k1-c/heat_otu_$tmp[0]vs$tmp[1]_sig.pdf)  [Heatmap noclust PDF](result_k1-c/heat_otu_$tmp[0]vs$tmp[1]_sig_noclust.pdf)  [Manhattan plot phlyum PDF](result_k1-c/man_otu_$tmp[0]vs$tmp[1].pdf) [Manhattan plot order PDF](result_k1-c/man_order_$tmp[0]vs$tmp[1].pdf)
...
figs_2 = paste0("result_k1-c/", c("heat_otu_$tmp[0]vs$tmp[1]_sig", "man_otu_$tmp[0]vs$tmp[1]"),".png")
knitr::include_graphics(figs_2)
```

$tmp[0]与$tmp[1]相比显著差异的OTU如，A_mean, B_mean分别为各组内相对丰度均值的百分比(%)，如 Table \\\@ref(tab:tab-otu-$tmp[0]vs$tmp[1]) 所示。[Enriched TXT](result_k1-c/otu_$tmp[0]vs$tmp[1]_enriched.xls)  [Depleted TXT](result_k1-c/otu_$tmp[0]vs$tmp[1]_depleted.xls)  [All TXT](result_k1-c/otu_$tmp[0]vs$tmp[1])  [Slime2 Adaboost Feature TXT](result_k1-c/ab_$tmp[0]-$tmp[1]_featimp_anno.txt)

```{r tab-otu-$tmp[0]vs$tmp[1]}
e = read.table("result_k1-c/otu_$tmp[0]vs$tmp[1]_enriched.txt", sep="\t", header=T)
e = head(e[order(-e\$A_mean),],n=10)
d = read.table("result_k1-c/otu_$tmp[0]vs$tmp[1]_depleted.txt", sep="\t", header=T)
d = head(d[order(-d\$B_mean),],n=10)
m = rbind(e,d)
m=m[,c("otu","A_mean","B_mean","logFC","PValue","phylum","class","order","family","genus")]
knitr::kable(m, row.names=F, caption="样品组$tmp[0] vs $tmp[1]相比显著差异的OTU详细信息；Significantlly different OTU.", booktabs=TRUE)
```
...
$file = "filter__k1/graphlan.pdf";
if (-e $file) {
print OUTPUT qq!
## 可培养OTUs

(ref:otu-graphlan) OTUs物种树，外圈颜色势图代表OTUs丰度，内圈黑块标注此OTUs是否存在97%以上相似的菌保，仅相对丰度0.1%的OTUs展示在本图中[PDF](filter__k1/graphlan.pdf)。[所有OTUs与培养菌COTU间关系](result/rep_seqs.blastn)。图中展示可培养数量和相比分析比例，详见[TXT](filter__k1/culture.txt)。
...
`cp -f figure/banner.png $opts{b}/figure/banner.png`;
`cp -rf result/average_plots $opts{b}/result`;
