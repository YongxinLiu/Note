...
#TR10000|c0_g1_i3        TR10000|c0_g1   1079    805.89  3171.00 45.80   110.32  24.69
	my @tmp=split/\t/;
	$database{$tmp[0]}{rpm}=$tmp[5];
	$database{$tmp[0]}{reads}=int($tmp[3]+0.5); # reads must integer
	$total[1]+=$tmp[3];
}
close DATABASE;
...
while (<DATABASE>) {
	chomp;
	my @tmp=split/\t/;
	$sample{$tmp[0]}{rpm}=$tmp[5];
	$sample{$tmp[0]}{reads}=int($tmp[3]+0.5);
	$total[0]+=$tmp[3];
}
close DATABASE;
...
sub usage {
    die(
        qq/
Usage:    compare_different_trinityiso.pl -i special -d WT -o output_file
...
Version:  v1.0
Update:   2016-03-07
Notes:    Update from compare_different_src.pl, only for trinity RSEM.isoforms.result
\n/
    )
}
