...
#		next unless $tmp2[5]>$overlap;
		if ($anno=~/RNA$|RNA\||\|tRNA/ && $tmp2[5]>$opts{d}*($tmp[4]-$tmp[3]+1) && $tmp2[5]>20) { #match all the TAIR annotated RNA(RNA$) and miRNA, overlap>30%,
			next if ($pointer==1 && $tmp2[5]<=$overlap);
			...
			$description=$tmp2[8];
			$pointer=1;
		}elsif ($anno=~/Repbase|mtec/ && $tmp2[5]>$opts{d}*($tmp[4]-$tmp[3]+1) && $tmp2[5]>20) { #repeat, and overlap>30%
			next if ($pointer==2 && $tmp2[5]<=$overlap);
			next if $pointer<2;
			$type="repeat";
			$overlap=$tmp2[5];
			$gi=$tmp2[0];
			...
			$description=$tmp2[8];
			$pointer=2;
		}elsif ($anno=~/transposable|pseudogene/ && $tmp2[5]>$opts{d}*($tmp[4]-$tmp[3]+1) && $tmp2[5]>20) { #TE, and overlap>30%
			next if ($pointer==3 && $tmp2[5]<=$overlap);
			next unless $tmp2[5]>$overlap;
			next if $pointer<3;
			$type="repeat";
			$overlap=$tmp2[5];
			$gi=$tmp2[0];
			$class=$tmp2[7];
			if ($anno=~/pseudogene/ ) {#Some photosythesis related gene annotated as protein_coidng_gene, classify into pseudogene
				$class="pseudogene";
			}
			$description=$tmp2[8];
			$pointer=3;
		}elsif ($anno=~/intron/ && $tmp2[5]>$opts{d}*($tmp[4]-$tmp[3]+1) && $tmp2[5]>20) { #intron, and overlap>30%
			next if ($pointer==4 && $tmp2[5]<=$overlap);
			next unless $tmp2[5]>$overlap;
			next if $pointer<4;
			$type="intron";
			$overlap=$tmp2[5];
...
#			print $description,"\n";
			...
			next if ($pointer==5 && $tmp2[5]<=$overlap);
			next unless ($anno=~/protein_coding_gene/);
			next unless $tmp2[5]>$overlap;
			next if $pointer<5;
			$type="exon";
			$overlap=$tmp2[5];
	...
	if ($pointer==6) {
		...
		print OUTPUT "$type\|$overlap\|$gi\|$class\|$description\n";
	}
}
...
sub usage {
    die(
        qq/
Usage:    Annotate_SRCs_overlap_simplify1.2.pl -i inpute_file -o output_file
Function: simplify overlap repeat region, repeat select long cover repeat, then select TE, pseudo
...
Version:  v1.2
Update:   2014-03-19
Notes:    TE and pseudogene region have repeat annotation, exon region fix annotation
		  Some photosythesis related gene annotated as protein_coidng_gene, classify into pseudogene
\n/
    )
}