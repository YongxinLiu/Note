...
while (<DATABASE>) {
	chomp;
	my @tmp=split/\t/;
	$database{$tmp[0]}=$tmp[9];
	$database{$tmp[0]}=($tmp[9]+$tmp[13])/2 if $opts{r}==2;
}
close DATABASE;
...
sub usage {
    die(
        qq/
Usage:    cuffdiff_homolog.pl -i homolog_list -o homolog_gene_exp -a AA_iso_exp -d DD_iso_exp -h header num
Function: Calculate homolog different
