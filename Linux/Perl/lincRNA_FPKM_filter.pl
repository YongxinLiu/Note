...
sub usage {
    die(
        qq/
Usage:    lincRNA_FPKM_filter.pl -i merged.strand.novel3.bed(过滤后的新转录本) -o output_file(按表达过滤的结果) -d database(表达数据) -e exon_number(外显子数量) -h header num
