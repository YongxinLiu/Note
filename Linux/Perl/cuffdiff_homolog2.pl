...
$opts{s}=13 unless defined($opts{s});
...
my %second; #second in hash
while (<DATABASE>) {
	chomp;
	my @tmp=split/\t/;
	$database{$tmp[0]}=$tmp[$opts{c}];
	$second{$tmp[0]}=$tmp[$opts{s}];
}
close DATABASE;
...
open OUTPUT,">$opts{o}";
#ID_AA	ID_DD	exp_AA	expDD	expADA	expADD	fold
#TRIUR3_00024-T1 EMT27083        0       0       0       0       1.0000
...
while (<INPUT>) {
	chomp;
	my @tmp=split/\t/;
	printf OUTPUT "$tmp[0]\t$tmp[1]\t$database{$tmp[0]}\t$database{$tmp[1]}\t$second{$tmp[0]}\t$second{$tmp[1]}\t%.4f\n",($second{$tmp[1]}+$second{$tmp[0]}+1)/($database{$tmp[1]}+$database{$tmp[0]}+1);
...
sub usage {
    die(
        qq/
Usage:    cuffdiff_homolog2.pl -i homolog_list -o homolog_gene_exp -a AA_iso_exp -d DD_iso_exp -h header num
Function: Calculate homolog total (AA+DD) different, can select samples line 9, 13, 17 AA-DD AADD DDAA
Command:  -i inpute file name (Must)
          ...
          -c column line form first, 9, 13, 17 represent sample1, 2, 3
          -s column line form second, 9, 13, 17 represent sample1, 2, 3
