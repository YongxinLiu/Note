...
while (<INPUT>) {
	...
	$tmp[1]=~s/k__(|unidentified);/k__unnamed;/;
	$tmp[1]=~s/p__(|unidentified);/p__unnamed;/;
	$tmp[1]=~s/c__(|unidentified);/c__unnamed;/;
	$tmp[1]=~s/o__(|unidentified);/o__unnamed;/;
	$tmp[1]=~s/f__(|unidentified);/f__unnamed;/;
	$tmp[1]=~s/g__(|unidentified);/g__unnamed;/;
	$tmp[1]=~s/s__(|unidentified)$/s__unnamed/;
	# 无分类级补Unassigned，代表无相似序列新种属
