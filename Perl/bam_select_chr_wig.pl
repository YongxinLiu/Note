...
my $bed_list;
foreach $chr(@tmp1) {
	`bamtools convert -in $opts{i} -region $chr -format bed -out $opts{o}$chr\.bed`;
	$bed_list.="$opts{o}$chr\.bed ";
}
#merge each chr, and delete temp
`cat $bed_list>$opts{o}\.bed`;
`rm $bed_list`;
$scale=1000000/$opts{r};
`bedToWig.py $opts{o}\.bed | awk 'BEGIN{OFS="\\t";FS="\\t"}{if(\$2!="") \$2=\$2*$scale; print \$0;}'>$opts{o}\.wig`;
