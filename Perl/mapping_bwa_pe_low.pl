...
print "bwa sampe $opts{g} $opts{o}\_left.sai $opts{o}\_right.sai $opts{l} $opts{r} -f $opts{o}\_pe.sam\n\n";
`bwa sampe $opts{g} $opts{o}\_left.sai $opts{o}\_right.sai $opts{l} $opts{r} -f $opts{o}\_pe.sam`; #-n 1 -N 1 for exclude repeat, only for chip-seq
...
print "Sam to sorted bam, filter unmapped result\n";
print "samtools view $opts{o}\_pe.sam -F4 -bS |samtools sort -  $opts{o}\n\n";
`samtools view $opts{o}\_pe.sam -F4 -bS|samtools sort -  $opts{o}`; # -q 20
...
sub usage {
    die(
        qq/
Usage:    mapping_bwa_pe_low.pl -l left.fq -r right.fq -g genome -t thread -q mapping_quality -o output_file_prefix
Function: mapping clean pair-end fastq to genome by bwa, not filter unique mapping -n 1 -N 1, not filter mapping quality  -q 20, deltect repeat region reads
