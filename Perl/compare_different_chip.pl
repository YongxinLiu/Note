...
#ID0	reads1	RPM2
#IGS6	467	16.7316800767067
#IGS7	0	0
#IGS8	8	0.286576071134486
while (<DATABASE>) {
	chomp;
	my @tmp=split/\t/;
	$database{$tmp[0]}{rpm}=$tmp[2];
	$database{$tmp[0]}{reads}=$tmp[1];
}
close DATABASE;
...
while (<DATABASE>) {
	chomp;
	my @tmp=split/\t/;
	$sample{$tmp[0]}{rpm}=$tmp[2];
	$sample{$tmp[0]}{reads}=$tmp[1];
}
close DATABASE;
...
#	my $p_value = &Fisher_Test($reads[0], $reads[0]+$total[0], $reads[0]+$reads[1], $reads[0]+$reads[1]+$total[0]+$total[1]);
	...
	$sample{$id}{pvalue} = &Fisher_Test($sample{$id}{reads}, $sample{$id}{reads}+$opts{a}, $sample{$id}{reads}+$database{$id}{reads}, $sample{$id}{reads}+$database{$id}{reads}+$opts{a}+$opts{b});
} 
...
foreach my $id(sort keys %sample) {
	printf OUTPUT "$id\t$sample{$id}{rpm}\t$database{$id}{rpm}\t%.2f\t$sample{$id}{pvalue}\n",$sample{$id}{fold};
} 
close OUTPUT;
...
sub usage {
    die(
        qq/
Usage:    compare_different_chip.pl -i chip1_treatment -d chip2_control -a chip1_reads -b chip2_reads -o output file
Function: compute chip_region RPM fold change, for reducing the low RPM caused big change or divide by zero, fold=treatment+1\/control+1
...
Command:  -i treatment samples
          ...
          -a treatment total reads
          -b control total reads
          -o output
