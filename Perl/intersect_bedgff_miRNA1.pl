...
my (%loci,$total,%reads,%rpm,%reads_all,%rpm_all);#reads and rpm were normalized by loci
while (<LOCI>) {
	chomp;
	my @tmp=split/\t/;
...
#		$rpm_all{$1}=$reads_all{$1}*1000000/$total;
		printf OUTPUT "$1\t$reads_all{$1}\t%.2f\n",$reads{$1};
	}else{
		print OUTPUT "$1\t0\t0\n";
	}
}
...
sub usage {
    die(
        qq/
Usage:    intersect_bedgff_miRNA.pl -i cluster -d intersect_gff_result -o RPM -t loci_file
Function: get the intersect result into reads and normalized reads
Command:  -i inpute file name (Must)
          -o output file name (Must)
          -d database file name
          -t database file name
Author:   Liu Yong-Xin, woodcorpse\@163.com, QQ:42789409
Version:  v1.0 get the normalized rpm
		  v2.0 add the rpm before normalized rpm
Update:   2013-05-20
Notes:    get the reads and normalized reads, no compute RPM
\n/
    )
}