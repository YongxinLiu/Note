...
#	print "max=$hp\tmin=$lp\n"
	if ($hybrid[$_]>=($mpv*(1-$opts{s})) && $hybrid[$_]<=($mpv*(1+$opts{s}))) {#MP
		...
		print OUTPUT "$id[$_]\t$paternal[$_]\t$maternal[$_]\t$hybrid[$_]\t$mpv\tOther\n";
	}
}
close OUTPUT;
...
sub usage {
    die(
        qq/
Usage:    hybrid_classify7.pl -p B1_cluster.rpkm -m A2_cluster.rpkm -h A3_cluster.rpkm -o B1_A2_A3.class -s 0.05
