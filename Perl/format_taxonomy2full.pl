...
while (<INPUT>) {
	...
	$tmp[1]=~s/k__(|unidentified);/k__unnamed${i};/;
	$tmp[1]=~s/p__(|unidentified);/p__unnamed${i};/;
	$tmp[1]=~s/c__(|unidentified);/c__unnamed${i};/;
	$tmp[1]=~s/o__(|unidentified);/o__unnamed${i};/;
	$tmp[1]=~s/f__(|unidentified);/f__unnamed${i};/;
	$tmp[1]=~s/g__(|unidentified);/g__unnamed${i};/;
	$tmp[1]=~s/s__(|unidentified)$/s__unnamed${i}/;
	# 无分类级补unknown，代表无相似序列新种属
	$tmp[1]="k__unknown${i}" unless ($tmp[1]=~/k__/);
	$tmp[1].=";p__unknown${i}" unless ($tmp[1]=~/p__/);
	$tmp[1].=";c__unknown${i}" unless ($tmp[1]=~/c__/);
	$tmp[1].=";o__unknown${i}" unless ($tmp[1]=~/o__/);
	$tmp[1].=";f__unknown${i}" unless ($tmp[1]=~/f__/);
	$tmp[1].=";g__unknown${i}" unless ($tmp[1]=~/g__/);
	$tmp[1].=";s__unknown${i}" unless ($tmp[1]=~/s__/);
